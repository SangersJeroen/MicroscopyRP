...
f_3_c = np.array([69.9,70.9,70.3,30.2,70.9])*1e-6
f_4_c = np.array([65, 65.1,65.1,65.2,65.6])*1e-6
...
delay_1 = np.array([260, 510, 600, 1160, 1760])*1e-9
delay_2 = np.array([260, 510, 600, 650, 1250])*1e-9
...
bf_2_avg = np.average(bf_2)

    # Error calculations
    
u_f = 1e-6
u_delay = np.array([30, 20, 20, 20, 30])*1e-9
...
data_2 = Data(D, delay_2, wd=1./u_D, we=1./u_delay)
...
out_2.pprint()
    ...
    # Plot the lot
x = np.linspace(0,3500e-9,200)
y_1 = func(x, 1/out_1.beta)
y_2 = func(x, 1/out_2.beta)
...
plt.plot(x, y_2, label="Birefringe 2")
    ...
    # Data
plt.errorbar(delay_1, D, xerr=u_delay, yerr=u_D, mec='k', linestyle='none',elinewidth=1,capsize=2)
plt.errorbar(delay_2, D, xerr=u_delay, yerr=u_D, mec='k', linestyle='none',elinewidth=1,capsize=2)
plt.legend()
plt.show()

print(''' Birefringence 1 = %.3e +- %.3e'''
      %(out_1.beta, out_1.sd_beta))
print(''' Birefringence 2 = %.3e +- %.3e'''
      %(out_2.beta, out_2.sd_beta))
